...
Module.STDWEB_PRIVATE.acquire_tmp = function( dummy ) {
    ...
    if( __load_asynchronously ) {
        return WebAssembly.instantiate( __wasm_module, __imports )
            .then( instance => {
                ...
                console.log( "Finished loading Rust wasm module 'safari_test'" );
                return Module.exports;
            })
            .catch( error => {
                console.log( "Error loading Rust wasm module 'safari_test':", error );
                throw error;
            });
    } else {
    ...
    if( typeof window === "undefined" ) {
        ...
        const wasm_path = path.join( __dirname, "safari-test.wasm" );
        const buffer = fs.readFileSync( wasm_path );
        const mod = new WebAssembly.Module( buffer );

        return __initialize( mod, false );
    } else {
        return fetch( "safari-test.wasm" )
